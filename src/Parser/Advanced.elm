module Parser.Advanced exposing
    ...
    ,  i
      , isSubString
      , k
        -- Exposed For Testing - isSubString
...
k : Parser c x a -> Parser c x (a -> b) -> Parser c x b
k parseArg parseFunc =
    keeper parseFunc parseArg
...
i : Parser c x ignore -> Parser c x keep -> Parser c x keep
i ignoreParser keepParser =
    ignorer keepParser ignoreParser
...
isSubString smallString offset row col bigString =
    let
        ...
        smallLength =
            Array.length smallStringArr
    in
    isSubStringHelp ( smallLength, smallStringArr, bigStringArr )
        ( offset + smallLength <= Array.length bigStringArr, 0, ( offset, row, col ) )


isSubStringHelp : ( Int, Array Char, Array Char ) -> ( Bool, Int, ( Int, Int, Int ) ) -> ( Int, Int, Int )
isSubStringHelp (( smallLength, smallStringArr, bigStringArr ) as triple) ( isGood, index, ( offset, row, col ) ) =
    if isGood && index < smallLength then
        isSubStringHelp triple
            (if isEqualTo ( index, smallStringArr ) ( offset, bigStringArr ) then
                let
                    maybeCode =
                        bigStringArr |> Array.get offset |> Maybe.map Char.toCode
                in
                if checkMaybe ((==) 0x0A) maybeCode then
                    ( True, index + 1, ( offset + 1, row + 1, 1 ) )

                else if checkMaybe (\code -> Bitwise.and code 0xF800 == 0xD800) maybeCode then
                    ( isEqualTo ( index + 1, smallStringArr ) ( offset + 1, bigStringArr )
                    , index + 2
                    , ( offset + 2, row, col + 1 )
                    )

                else
                    ( True, index + 1, ( offset + 1, row, col + 1 ) )

             else
                ( False, index + 1, ( offset + 1, row, col ) )
            )
...
checkMaybe : (value -> Bool) -> Maybe value -> Bool
checkMaybe func maybeValue =
    Maybe.withDefault False (Maybe.map func maybeValue)


isEqualTo : ( Int, Array Char ) -> ( Int, Array Char ) -> Bool
isEqualTo ( x, xA ) ( y, yA ) =
    Maybe.withDefault False (Maybe.map2 (==) (Array.get x xA) (Array.get y yA))
...
variable config =
    Parser <|
        \s ->
            let
                firstOffset =
                    isSubChar config.start s.offset s.src
            in
            if firstOffset == -1 then
                Bad False (fromState s config.expecting)

            else
                let
                    s1 =
                        if firstOffset == -2 then
                            varHelp config.inner (s.offset + 1) (s.row + 1) 1 s.src s.indent s.context

                        else
                            varHelp config.inner firstOffset s.row (s.col + 1) s.src s.indent s.context

                    name =
                        String.slice s.offset s1.offset s.src
                in
                if Set.member name config.reserved then
                    Bad False (fromState s config.expecting)
...
sequence :
    ...
    -> Parser c x (List a)
sequence config =
    skip (token config.start) <|
        skip config.spaces <|
            sequenceEnd (token config.end) config.spaces config.item (token config.separator) config.trailing
